...
import { deleteAdminHighlightById } from '../../../../utils/highlights'
...
export default defineEventHandler(async (event): Promise<AdminHighlightResponse> => {
  ...
  try {
    ...
    const highlight = await deleteAdminHighlightById(session, highlightId)
    ...
    const statusMessage =
      ...
      error.statusMessage
        ? error.statusMessage
        : 'Nao foi possivel remover o destaque.'

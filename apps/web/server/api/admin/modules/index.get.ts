import type { AdminModulesResponse, AuthSessionContext } from '@ieb/shared'
import { defineEventHandler, setResponseStatus } from 'h3'
import { requireAuthSession, writeAdminLog } from '../../../utils/auth'
import { listAdminModulesForManagement } from '../../../utils/courses'

export default defineEventHandler(async (event): Promise<AdminModulesResponse> => {
  let session: AuthSessionContext | null = null

  try {
    session = await requireAuthSession(event, { admin: true })
    const modules = await listAdminModulesForManagement(session)
    ...
    const statusMessage =
      ...
      error.statusMessage
        ? error.statusMessage
        : 'Nao foi possivel carregar os modulos do painel.'
